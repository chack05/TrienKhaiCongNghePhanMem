...
// Kiểm tra nếu session có user_id, nếu không, yêu cầu người dùng đăng nhập
if (!isset($_SESSION['user_id'])) {
    header('Location: login.php'); // Chuyển hướng người dùng đến trang đăng nhập
    exit();
}

$userId = $_SESSION['user_id'];  // Lấy user_id từ session, không phải từ cookie

// Lấy truyen_id từ URL
$truyen_id = $_GET['truyen_id'];

// Kiểm tra nếu truyen_id hợp lệ
if (empty($truyen_id) || !is_numeric($truyen_id)) {
    die("Truyện không hợp lệ!");
}
...
// Lưu thông tin vào lịch sử xem truyện
$query = "INSERT INTO history (user_id, truyen_id, viewed_at) VALUES (?, ?, NOW())";
$stmt = mysqli_prepare($conn, $query);
mysqli_stmt_bind_param($stmt, 'ii', $userId, $truyen_id);

// Kiểm tra và thực thi câu lệnh
if (!mysqli_stmt_execute($stmt)) {
    die("Lỗi khi thêm lịch sử xem: " . mysqli_error($conn));
}

// Lấy danh sách chương của truyện
$sqlChuong = "SELECT * FROM chuong WHERE truyen_id = $truyen_id";
